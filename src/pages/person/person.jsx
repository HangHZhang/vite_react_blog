...
export default function Person() {
  ...
  const { username, phone, create_at, email, center, school, brith, idCard, politicalStatus, studentNumber, place, research, college} = user.userInfo;
    ...
    notification.open({
      ...
      description: (
        <Descriptions layout="vertical">
          ...
          <Descriptions.Item label="出生地"> {brith}</Descriptions.Item>
          <Descriptions.Item label="创建时间">
            {moment(+(create_at || '533896703232')).format(
              "YYYY-MM-DD HH:mm:ss"
            )}
          </Descriptions.Item>
          <Descriptions.Item label="院校">{school || 'xxx'}</Descriptions.Item>
          ...
          <Descriptions.Item label="学号">{studentNumber || 'xxx'}</Descriptions.Item>
          ...
          <Descriptions.Item label="学院">{college || 'xxx'}</Descriptions.Item>
        </Descriptions>
      ),
      style: {
        width: "40%",
  ...
  const handleSubmit = async (values) => {
    if (values.confirmPassword) {
      const result = reqUpdatePassword(phone, values.oldPassword, values.newPassword);
      result.then((res) => {
        if (res.status === 0) {
    ...
    <Row gutter={10}>
      <Col span={7}>
        ...
        >
          ...
          <LeftRight left="用户名称" right={username || 'xxx'} icon={<IconUser />}/>
          <hr />
          <LeftRight left="用户邮箱" right={email || 'xxx'} icon={<IconEmail />}/>
          <hr />
          <LeftRight left="手机电话" right={phone || 'xxx'} icon={<IconPhone />}/>
          <hr />
          <LeftRight
            left="创建时间"
            right={moment(+(create_at || '533896703232')).format(
              "YYYY-MM-DD HH:mm:ss"
            )}
            icon={<IconLineHeight />}
          />
          <hr />
          <LeftRight left="所属中心" right={center || 'xxx'}  icon={<IconCopyright />}/>
          <hr />
          <LeftRight left="在读院校" right={school || 'xxx'} icon={<IconTag />}/>
